...
class StringType extends AbstractType implements TypeInterface
{
    ...
    {
        if (!is_null($min)) {
            $this->addCustom(static function (mixed $value) use ($min) : bool {
                return mb_strlen($value) >= $min ?: throw new ValidationException('string length', '>= ' . $min, mb_strlen($value));
            });
        }

        if (!is_null($max)) {
            $this->addCustom(static function (mixed $value) use ($max) : bool {
    ...
    {
        return $this->addCustom(static function (mixed $value) use ($regexp, $regexpTitle) : bool {
    ...
    {
        return $this->addCustom(static function (mixed $value) : bool {
