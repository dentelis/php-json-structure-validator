...
class IntegerType extends AbstractType implements TypeInterface
{
    ...
    {
        if (!is_null($min)) {
            $this->addCustom(static function (int $value) use ($min) : bool {
                return ($value) >= $min ?: throw new ValidationException('value', '>=' . $min, ($value));
            });
        }

        if (!is_null($max)) {
            $this->addCustom(static function (int $value) use ($max) : bool {
    ...
    {
        $this->addCustom(static function (int $value) : bool {
